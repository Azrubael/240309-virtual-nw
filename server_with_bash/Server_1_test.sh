...
function create_vm {
    ...
    VBoxManage guestcontrol $VM_NAME copyto 70-netplan-config.yaml /tmp/70-netplan-config.yaml --username server1 --password server1
    VBoxManage guestcontrol $VM_NAME copyto server1_setup.sh /tmp/server1_setup.sh --username server1 --password server1
    VBoxManage --nologo guestcontrol $VM_NAME run --exe /tmp/server1_setup.sh --username server1 --password server1 --wait-stdout
